...
void EnergyMonitor::current2(unsigned int _inPinI, double _ICAL)
{
  inPinI2 = _inPinI;
  ICAL2 = _ICAL;
...
void EnergyMonitor::calcVI(unsigned int crossings, unsigned int timeout)
{
  ...
  while ((crossCount < crossings) && ((millis()-start)<timeout))
  {
    ...
    //-----------------------------------------------------------------------------
    instP = phaseShiftedV * (filteredI+filteredI2);          //Instantaneous Power
    sumP +=instP;                               //Sum
  ...
  realPower = V_RATIO * (I_RATIO+I_RATIO2)/2 * sumP / numberOfSamples;
